...
func checkContainers(pod *corev1.Pod, events *corev1.EventList) []DiagnosisResult {
	...
	for i := range pod.Status.ContainerStatuses {
		...
		results = append(results, checkOOMKilled(cs, pod)...)
...
// either in its current state or its most recent termination.
func checkOOMKilled(cs *corev1.ContainerStatus, pod *corev1.Pod) []DiagnosisResult {
	...
	memLimit := memoryLimit(cs.Name, pod)
...
// memoryLimit returns the memory limit string for the named container, or empty
// if the container has no limit set.
func memoryLimit(containerName string, pod *corev1.Pod) string {
